...
import IProductFavoriteRepository from './IProductFavoriteRepository'
...
@singleton()
export default class FirestoreProductFavoriteRepository implements IProductFavoriteRepository {
  private readonly _collectionName = firestoreConfig.productFavorite as string
  private readonly _collectionRef: CollectionReference<ProductFavoriteDto>
  ...
  async paging (limit: number, after: string, before: string): Promise<PagingResult<ProductFavoriteDto>> {
    ...
    const pagingResult: PagingResult<ProductFavoriteDto> = {
      data: all,
      pagination: {
  ...
  async findById (id: string): Promise<ProductFavoriteDto | null> {
    const documentRef = await this._collectionRef.doc(id).get()
    if (documentRef.exists) {
      const one: ProductFavoriteDto = {
        id: documentRef.id,
        userId: documentRef.data()?.userId,
        productId: documentRef.data()?.productId,
        productName: documentRef.data()?.productName,
        productImage: documentRef.data()?.productImage,
        productDescription: documentRef.data()?.productDescription,
