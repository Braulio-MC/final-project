...
import ProductReviewService from '../data/service/ProductReview.service'
...
@singleton()
export default class ProductReviewController implements IController {
  constructor (
    @inject(ProductReviewService) private readonly service: ProductReviewService,
    @inject(ProductReviewDtoBuilder) private readonly builder: ProductReviewDtoBuilder
  ) {}
  ...
  async create (req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if (errors.isEmpty()) {
      ...
      const newProductReview = this.builder
        .setUserId(userId)
  ...
  async update (req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if (errors.isEmpty()) {
      ...
      const at = Timestamp.fromDate(moment().toDate())
      const updateProductReview = this.builder
        .setRating(rating)
        .setUpdatedAt(at)
        .build()
      this.service.update(id, updateProductReview)
        .then(_ => {
  ...
  async paging (req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if (errors.isEmpty()) {
      try {
        ...
        const productReviews = await this.service.paging(limit, after, before)
